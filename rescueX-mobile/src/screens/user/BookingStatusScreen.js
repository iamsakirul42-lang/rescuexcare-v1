...
export default function BookingStatusScreen({ navigation, route }) {
  ...
  // --- MVP Mock Transitions (To demonstrate the flow without an admin panel) ---
  useEffect(() => {
    let timeout;
    if (status === 'waiting_assignment') {
      // Simulate admin dispatching expert after 5s
      timeout = setTimeout(() => {
        animateTransition(() => {
          updateBookingStatus(bookingId, 'expert_assigned', {
            expert: {
              name: 'Rajesh Kumar',
              phone: '+91 9876543210',
              specialization: 'Four Wheeler Expert',
              experience: '5 Years',
              rating: 4.8
            }
          });
    ...
    else if (status === 'expert_assigned') {
      // Move to on_way automatically for demo
      timeout = setTimeout(() => animateTransition(() => updateBookingStatus(bookingId, 'expert_on_way')), 4000);
    }
    else if (status === 'expert_on_way') {
      // Move to arrived automatically for demo
      timeout = setTimeout(() => animateTransition(() => updateBookingStatus(bookingId, 'expert_arrived')), 8000);
    }
      ...
      case 'complete_verification':
        ...
        return (
          <View style={{ flex: 1, alignItems: 'center', width: '100%', paddingBottom: 40 }}>
            ...
            {/* Bottom: Timer & OTP in a transparent box */}
            <View style={{ width: '100%', alignItems: 'center', justifyContent: 'flex-end' }}>
              <View style={[styles.codeContainer, { width: '100%', alignItems: 'center', backgroundColor: 'transparent', marginVertical: 0, paddingVertical: 20 }]}>
                ...
                <Text style={[styles.statusSubtitle, { marginTop: 4, marginBottom: 8, fontSize: 13 }]}>Share this code when work is done.</Text>
                <View style={{ marginVertical: 4 }}>
                  <Text style={[styles.codeText, { fontSize: 32 }]}>{completionCode}</Text>
                </View>
                
                <TouchableOpacity style={[styles.primaryButton, { marginTop: 12 }]} onPress={handleConfirmCompletion}>
